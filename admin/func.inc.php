...
{	
	...
	if ($action == 'get') {	
		foreach($sort1 as $i => $key)
		{
			...
			{
				$menu .= '<div class="well well-small" ' . $style . '>';
				$op[] = $key;
			}			

			$menu .= '<p><a class="add_inputs" href="#" onclick="return moreInputs(' . $temp_parent[$sort1[$i]] . ', ' . $temp_index[$sort1[$i]] . ', (this))">Добавить ещё пункт</a></p>';
						
			$tmp_index = $index_pre[$key] . $index[$key];

			$style = (empty($style)) ? 'style="background-color:white;"' : '';
			$menu .= '
			<div class="well well-small control-group"' . $style . '>		
					<input type="text" name="menu' . $tmp_index . '[0]" class="span3" value="' . $temp_menu_item[$key][0] . '" />
					<input type="text" name="menu' . $tmp_index . '[1]" class="span4" value="' . $temp_menu_item[$key][1] . '" />
					<input type="text" name="menu' . $tmp_index . '[2]" class="span4" value="' . $temp_menu_item[$key][2] . '" />
					<a href="#" title="Удалить пункт меню" onclick="return deleteInputs(this);" class="delete">удалить</a>
			</div>';
			$menu .= (isset($temp_menu_item[$key][3])) ? '' : 
			'<p><a class="add_blocks" href="#" onclick="return moreBlocks(' . $temp_parent[$sort1[$i]] . ', ' . $temp_index[$sort1[$i]] . ', (this))">Добавить дочернее меню</a></p>';
			if (!isset($temp_menu_item[$key][3]))
			{
				...
				{
					$last = $c - 1;
					$menu .= '<p><a class="add_inputs" href="#" onclick="return moreInputs(' . $temp_parent[$op[$last]] . ', ' . count($tmp_item[$temp_parent[$op[$last]]]) . ', (this))">Добавить ещё пункт</a></p>';
					$menu .= '</div>';
		...
		$menu .= '<p><a class="add_inputs" href="#" onclick="return moreInputs(0, ' . count($tmp_item[0]) . ', (this))">Добавить ещё пункт</a></p>';
	}
	#SET
	else {	
	$menu .= 'array (';
