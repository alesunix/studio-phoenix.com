	...
	if (isset($_POST['rename'])) ### переименование файла
	{
		...
		{	# выводим форму
			...
			<p>
				...
				Адрес файла: <strong>' . $site . $ifolder . '/' . $_POST['newname'] . ' (<a href="' . $site . $ifolder . '/' . $_POST['newname'] . '" target="_blank">просмотр</a>)</strong>
			</p>
			<hr>
			<p class="t-center">
				<a href="javascript:toOpen(\'' . $site . $ifolder . '/' . $_POST['newname'] . '\')">Вставить изображение</a> <a href="?">Загрузить ещё один файл!</a>
			</p>';
	...
	elseif (isset($_POST['replace'])) ### замена файла
	{
		...
		{	# выводим форму
			...
			<p>
				Тип файла: <strong>' . $_POST['oldtype'] . '</strong><br>
				Размер файла: <strong>' . round($_POST['oldsize'] / 1024, 2) . ' кб.</strong><br>
				Адрес файла: <strong>' . $site . $ifolder . '/' . $_POST['name'] . ' (<a href="' . $site . $ifolder. '/' . $_POST['name'] . '" target="_blank">просмотр</a>)</strong>
			<p>
			<hr>
			<p class="t-center">
				<a href="javascript:toOpen(\'' . $site . $ifolder . '/' . $_POST['name'] . '\')">Вставить изображение</a> <a href="?">Загрузить ещё один файл!</a>
			</p>';
	...
	else
	{
		...
		else # проверка на существование файла в папке на сервере
		{
			if (file_exists($path . '/' . $_FILES['UserFile']['name']))
			{
				...
				<hr>
				<p>
					Адрес файла: <strong>' . $site . $ifolder . '/' . $_FILES['UserFile']['name'] . ' <a href="' . $site . $ifolder . '/' . $_FILES['UserFile']['name'] . '" target="_blank">Ссылка</a></strong> 
			...
			else # если всё ок...
			{
				...
				else
				{
					...
					<p>
						...
						Адрес файла: <strong>' . $site . $ifolder . '/' . $_FILES['UserFile']['name'] . ' (<a href="' . $site .  $ifolder . '/' . $_FILES['UserFile']['name'] . '" target="_blank">просмотр</a>)</strong>
					</p>
					<hr>
					<p class="t-center">
						<a href="javascript:toOpen(\'' . $site . $ifolder . '/' . $_FILES['UserFile']['name'] . '\')">Вставить изображение</a> <a href="?">Загрузить ещё один файл!</a>
					</p>';
				}
			}
		}
...
else
{
	...
	if (empty($_FILES['UserFile']['tmp_name']))
	{
		$content = '
		<p class="t-center">
			<strong>Загрузка изображений на сервер</strong>
		</p>
		<hr>
		<form method="post" enctype="multipart/form-data">
		...
		<p>
			...
			Папка загрузки: <strong>' . $site . $ifolder.'</strong>
		</p>';
	}
}
...
<head>
	...
	function toOpen(alias)
	{
		if (!opener)
			return;
		opener.document.getElementById("text").value += "<img src=\"" + alias + "\" alt=\"\">";
		self.close();
	}
	</script>
</head>
